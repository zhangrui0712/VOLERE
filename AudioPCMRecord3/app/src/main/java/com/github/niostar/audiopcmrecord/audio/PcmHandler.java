...
public class PcmHandler {
    ...
    public static void normalizePCM(float[] originalSignal) {
    ...
    public static float[] handleEndPoint(float[] originalSignal, int samplingRate) {

        int samplePerFrame = samplingRate / 1000;
        int firstSamples = samplePerFrame * 200;

        float[] voiced = new float[originalSignal.length];
        // each
        // sample whether it is voiced or unvoiced
        float sum = 0;
        double sd = 0.0;
        double m = 0.0;

        //too shot return null
        if (originalSignal.length < firstSamples) {
            return null;
        }
